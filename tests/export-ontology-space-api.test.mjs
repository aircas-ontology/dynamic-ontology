...
test("export ontology space api types cover the required spaceId query", () => {
  ...
  assert.match(typeBarrelSource, /export type \{ ExportOntologySpaceFile, ExportOntologySpaceParams \} from "\.\/apis\/exportOntologySpaceType";/);
});
...
test("space export confirmation calls the export api instead of local json", () => {
  ...
  assert.match(actionsSource, /getExportOntologySpaceInterface\(\{ spaceId \}\)/);
  assert.doesNotMatch(actionsSource, /serializeSpace/);
  assert.match(dialogSource, /分类树、本体 schema 与实例数据/);
