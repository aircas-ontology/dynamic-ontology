<template>
  <header class="header-bar">
    ...
    <div class="header-bar__search" title="全局搜索暂未开放">
      <el-input class="aircas-input" placeholder="检索空间、对象、实例、属性..." :prefix-icon="Search" ariaLabel="全局搜索（暂未开放）"
        disabled />
    </div>
    <div class="header-bar__tools">
      ...
      <button type="button" class="header-bar__tool" aria-label="消息暂未开放" title="消息暂未开放" disabled><el-icon :size="20">
          <Bell />
        </el-icon></button>
      <button type="button" class="header-bar__tool" aria-label="帮助暂未开放" title="帮助暂未开放" disabled><el-icon :size="20">
          <QuestionFilled />
        </el-icon></button>
      <span class="header-bar__user"><el-icon :size="20">
          <UserFilled />
        </el-icon><span>访客</span></span>
    </div>
  </header>
</template>
