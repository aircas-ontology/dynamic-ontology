# Views 目录规范

## 1. 目录职责与边界

`src/views` 存放路由页面。页面负责组合布局、组件、Store 和 API，编排完整用户流程。

- 【必须】页面只保留页面编排和页面私有逻辑，可复用能力下沉到对应公共目录。
- 【禁止】在页面中重复实现公共组件、请求基础设施、通用工具或跨页面状态。
- 【优先】页面私有子组件与页面同目录维护，确认复用后再提升到 `src/components`。

## 2. 结构与命名

```text
src/views/
└─ <PageName>/
   ├─ components/
   └─ index.vue
```

- 【必须】页面目录和页面组件使用 PascalCase，入口使用生态约定的 `index.vue`。
- 【必须】页面专属资源统一放入 `src/assets/pages/<pageName>`，其中资源目录使用小驼峰。
- 【禁止】新建 `src/views/<PageName>/assets`。
- 【优先】页面私有组件目录只在确有拆分需要时创建。

## 3. Vue、类型与依赖

- 【必须】使用 Vue 3 Composition API 和 `<script setup lang="ts">`。
- 【必须】页面业务类型从 `@/types` 导入，纯类型使用 `import type`。
- 【必须】接口通过 `@/apis` 使用，公共状态通过对应 Pinia Store 使用。
- 【优先】按 Vue、第三方包、项目模块、相对模块分组导入并保持稳定顺序。

## 4. 页面状态与数据流

- 【必须】异步页面处理 loading、empty、error 和成功状态。
- 【必须】避免相同请求在进行中重复触发，并防止过期响应覆盖新状态。
- 【优先】页面卸载时取消不再需要的请求、订阅和后台任务。
- 【禁止】静默吞掉异常或仅在控制台输出用户需要知道的失败。

## 5. UI、交互与可访问性

- 【必须】使用现有 Aircas 主题变量、公共组件和 Element Plus 覆盖。
- 【禁止】业务样式硬编码颜色；私有样式使用 `<style scoped lang="scss">`。
- 【必须】表单具有校验、提交中状态、失败提示和重复提交控制。
- 【必须】可操作元素使用正确语义，图片、图标按钮、键盘焦点和长文本具有可访问处理。
- 【必须】检查 normal、hover、active、disabled、focus 和 overflow 状态。

## 6. 第三方实例与生命周期

- 【必须】地图、图表和 Canvas 实例避免 Vue 深度响应式，只初始化一次并在卸载时销毁。
- 【必须】移除所有事件监听器、定时器、动画帧、Observer、Overlay 和订阅。
- 【必须】重建实例前执行与卸载相同的清理流程。

## 7. 变更检查清单

1. 页面目录、入口、私有组件和资源位置是否正确。
2. 页面是否只做流程编排，公共能力是否放在正确目录。
3. 异步四态、请求并发、错误反馈和卸载取消是否处理。
4. 路由名称和跳转目标是否存在且一致。
5. 主题、交互状态、无障碍和第三方实例清理是否验证。

## 8. 现状差异与后续整改

- `src/views/ontology` 使用小写目录且页面为空，后续应按真实页面职责改为 PascalCase 并补齐或移除。
- `LoginPage/assets` 后续应迁移到 `src/assets/pages/loginPage`，并统一文件与 import 路径大小写。
- 登录页当前包含硬编码演示账号、明文保存密码和占位令牌逻辑，后续认证任务必须移除并使用真实安全流程。
- `LoginPage/index.vue` 职责和样式体量较大，后续可按表单、品牌展示和视觉区域评估拆分。

