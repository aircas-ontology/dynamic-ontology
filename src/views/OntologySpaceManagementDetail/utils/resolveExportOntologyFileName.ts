interface ExportOntologyFileNameInput {
...
/**
 * @description 在响应头没有文件名时，用对象 API 名称和内容类型生成下载名。
 * @param apiName 对象 API 名称。
 * @param uniqueIdentifier 本体唯一标识。
 * @param contentType 响应 Content-Type。
 * @returns 备用下载文件名。
 */
function buildFallbackExportFileName(apiName: string, uniqueIdentifier: string, contentType: string): string {
  const base = apiName.trim() || `ontology-${uniqueIdentifier}`;
...
/**
 * @description 决定本体对象导出文件的下载名。
 * @param input 响应头与当前对象信息。
 * @returns 用于浏览器下载的文件名。
 */
export function resolveExportOntologyFileName(input: ExportOntologyFileNameInput): string {
  const fromHeader = sanitizeExportFileName(readDispositionFileName(input.contentDisposition));
  if (fromHeader) return fromHeader;
  return buildFallbackExportFileName(input.apiName, input.uniqueIdentifier, input.contentType);
}
