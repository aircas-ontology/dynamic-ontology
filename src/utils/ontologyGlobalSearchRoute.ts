...
/**
 * @description 将对象检索结果与对象简要信息拼成对象详情「对象」Tab 路由。
 * @param item 检索结果条目，需含 spaceId。
 * @param meta 按 objectId 查询得到的对象简要信息。
 * @returns 对象详情对象 Tab 路由；缺少必要字段时返回 null。
 */
export function resolveOntologyGlobalSearchObjectRoute(item: OntologyGlobalSearchItem, meta: GetOntologyMetaByObjectIdData): RouteLocationRaw | null {
  if (item.type !== "对象") {
    return null;
  }
  if (item.spaceId === undefined || item.spaceId === null || !Number.isFinite(item.spaceId)) {
