const MAP_CONFIG = {
  scene: {
    ...
    scene3DOnly: false, // 一般为false 如果为true 每个几何实例将仅以3D渲染以节省GPU内存
    shadows: false, // 是否启用日照阴影
    removeDblClick: true, // 是否移除cesium默认双击事件
    sceneMode: 3, // 初始场景模式
    showSun: true, // 是否显示太阳
    showMoon: true, // 是否显示月亮
    showSkyBox: true, // 是否显示天空盒
    showSkyAtmosphere: true, // 是否显示大气层外光圈
    shouldAnimate: true, // 是否开启时钟动画
    fog: true, // 是否启用雾化
    fxaa: true, // 是否开启快速抗锯齿
    verticalExaggeration: 1, // 地形夸张倍率
    orderIndependentTranslucency: true, // 如果为true 并且配置支持它 则使用顺序无关的半透明性
    requestRenderMode: false, // 是否显式渲染
    highDynamicRange: false, // 是否关闭高动态范围渲染 （为true时 地图变亮一些）（为false时 底图变暗一些）
    contextOptions: {
      requestWebgl1: false,
    },
    globe: {
      show: true, // 是否显示地球
      depthTestAgainstTerrain: false, // 是否开启深度监测 开启后，可以测试矢量对象是否在地形下面或被遮挡
      baseColor: "#546a53", // 地球背景色
      showGroundAtmosphere: true, // 是否在地球上绘制地面大气
      enableLighting: false, // 是否显示晨昏线，可以看到地球的昼夜区域
      tileCacheSize: 100, // 地形图块缓存的大小
      // terrainExaggeration: 1, // 地形夸张倍率
      realAlt: false,
    },
    cameraController: {
      zoomFactor: 3,
      minimumZoomDistance: 1,
      maximumZoomDistance: 1000000000,
      enableRotate: true, // 是否允许用户旋转相机
      enableTranslate: true, // 在2D和2.5D时，是否允许用户平移相机
      enableTilt: true, // 在2.5D和3D时，是否允许用户倾斜相机
      enableZoom: true, // 是否允许用户放大缩小
      enableCollisionDetection: true, // 是否允许地形相机碰撞检测
      minimumCollisionTerrainHeight: 15000, // 最小碰撞高度，低于此高度时绕鼠标键绕圈，大于时绕视图中心绕圈
    },
  },
  control: {
    toolbar: {
